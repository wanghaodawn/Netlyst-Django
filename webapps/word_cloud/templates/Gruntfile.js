...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      dist: {
        files: [
          ...
          { expand: true,
            src: '../static/assets/canvas-to-blob/canvas-to-blob.min.js',
            dest: '<%= distDir %>' }
        ]
      }
    },
    ...
    uglify: {
      ...
      dist: {
        files: [{
          src: '../static/assets/go2/src/google-oauth2.js',
          dest: '<%= distDir %>/assets/go2/src/google-oauth2.js'
        },
        {
          src: '../static/assets/wordfreq/src/wordfreq.worker.js',
          dest: '<%= distDir %>/assets/wordfreq/src/wordfreq.worker.js'
        },
        {
          src: '../static/assets/downloader-worker.js',
  ...
  grunt.registerTask('checkvars', function() {
    ...
    var fs = require('fs');
    fs.exists('../static/assets/vars.js', function(exists) {
      if (!exists) {
        grunt.fail.fatal('../static/assets/vars.js does not exist.');
      }
  ...
  grunt.registerTask('check-imgur-credit', function checkImgurCredit() {
    ...
    var IMGUR_CLIENT_ID =
      (fs.readFileSync('../static/assets/vars.js', {
